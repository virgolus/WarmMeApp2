	...
	while(1)
	{
		...
		while ( network.available() ) {
			...
			if (header.type == 't') {
				...
				// Elimina quando id sarà valorizzato da sensore temp
				message_resp resp;
				resp.temperature = message.temperature;
				resp.humidity = message.humidity;
				resp.id = message.id;

				sprintf (buffer, "mosquitto_pub -d -t sensors -m '{\"id\":%d, \"temperature\":%.2f, \"humidity\":%.2f}'", resp.id, resp.temperature,resp.humidity);
				// "system" is how you execute a shell command from a C program
				system(buffer);
			} else {
